...
export class SchedulerService {
  ...
  async sendDailyReport() {
    ...
    await this.discordService.sendAdminAlert({
      ...
      color: 0x4f46e5,
      fields: [
        { name: '🎓 운영 중 강의', value: `${stats.totalCourses}개`, inline: true },
        { name: '👥 전체 회원', value: `${stats.totalUsers}명`, inline: true },
        { name: '📚 오늘 수강 신청', value: `${stats.todayEnrollments}건`, inline: true },
        {
          name: '💰 오늘 매출',
          value: stats.todayRevenue === 0 ? '없음' : `${stats.todayRevenue.toLocaleString()}원`,
          inline: true,
        },
        { name: '⏳ 미결제 주문', value: `${stats.pendingOrders}건`, inline: true },
      ],
    });
  }
}
